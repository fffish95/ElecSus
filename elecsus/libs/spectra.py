...
p_dict_defaults = { 'Elem':'Rb', 'Dline':'D2', 
                            ...
                            # Lithium spectrum fitting
                            'DoppTemp_li6':250., 'DoppTemp_li7':250.,
...
def add_voigt_D1D2(d,DoppTemp,atomMass,wavenumber,gammaD1,gammaD2,voigtwidth,ltransnoD1,lenergyD1,lstrengthD1,
        rtransnoD1,renergyD1,rstrengthD1,
        ztransnoD1,zenergyD1,zstrengthD1,
        ltransnoD2,lenergyD2,lstrengthD2,
        rtransnoD2,renergyD2,rstrengthD2,
        ztransnoD2,zenergyD2,zstrengthD2):
    xpts = len(d)
    npts = 2*voigtwidth+1
    detune = 2.0*pi*1.0e6*(arange(npts)-voigtwidth) # Angular detuning (2pi Hz)
    wavenumber =  wavenumber + detune/c # Allow the wavenumber to change (large detuning)
    ...
    a_D2 = gammaD2/ku
    b = detune/ku
    y_D1 = 1.0j*(0.5*sqrt(pi)/ku)*wofz(b+0.5j*a_D1)
    y_D2 = 1.0j*(0.5*sqrt(pi)/ku)*wofz(b+0.5j*a_D2)
    ...
    disp_D1 = y_D1.real
    #interpolate lineshape functions
    f_ab_D1 = interp1d(detune,ab_D1)
    f_disp_D1 = interp1d(detune,disp_D1)
    f_ab_D2 = interp1d(detune,ab_D2)
    f_disp_D2 = interp1d(detune,disp_D2) 
    #Add contributions from all transitions to user defined detuning axis
    lab = zeros(xpts)
    ldisp = zeros(xpts)
    for line in range(ltransnoD1+1):
        xc = lenergyD1[line]
        lab += lstrengthD1[line]*f_ab_D1(2.0*pi*(d-xc)*1.0e6)
        ldisp += lstrengthD1[line]*f_disp_D1(2.0*pi*(d-xc)*1.0e6)
    for line in range(ltransnoD2+1):
        xc = lenergyD2[line]
        lab += lstrengthD2[line]*f_ab_D2(2.0*pi*(d-xc)*1.0e6)
        ldisp += lstrengthD2[line]*f_disp_D2(2.0*pi*(d-xc)*1.0e6)
    rab = zeros(xpts)
    rdisp = zeros(xpts)
    for line in range(rtransnoD1+1):
        xc = renergyD1[line]
        rab += rstrengthD1[line]*f_ab_D1(2.0*pi*(d-xc)*1.0e6)
        rdisp += rstrengthD1[line]*f_disp_D1(2.0*pi*(d-xc)*1.0e6)
    for line in range(rtransnoD2+1):
        xc = renergyD2[line]
        rab += rstrengthD2[line]*f_ab_D2(2.0*pi*(d-xc)*1.0e6)
        rdisp += rstrengthD2[line]*f_disp_D2(2.0*pi*(d-xc)*1.0e6)
    zab = zeros(xpts)
    zdisp = zeros(xpts)
    for line in range(ztransnoD1+1):
        xc = zenergyD1[line]
        zab += zstrengthD1[line]*f_ab_D1(2.0*pi*(d-xc)*1.0e6)
        zdisp += zstrengthD1[line]*f_disp_D1(2.0*pi*(d-xc)*1.0e6)
    for line in range(ztransnoD2+1):
        xc = zenergyD2[line]
        zab += zstrengthD2[line]*f_ab_D2(2.0*pi*(d-xc)*1.0e6)
        zdisp += zstrengthD2[line]*f_disp_D2(2.0*pi*(d-xc)*1.0e6)
    return lab, ldisp, rab, rdisp, zab, zdisp
...
def calc_chi(X, p_dict,verbose=False):             
    ...
    if 'DoppTemp_li6' in list(p_dict.keys()):
        DoppTemp_li6 = p_dict['DoppTemp_li6']
    else:
        DoppTemp_li6 = p_dict_defaults['DoppTemp_li6']
    if 'DoppTemp_li7' in list(p_dict.keys()):
        DoppTemp_li7 = p_dict['DoppTemp_li7']
    else:
        DoppTemp_li7 = p_dict_defaults['DoppTemp_li7']
    ...
    if Elem=='Li':
        Li7frac=1.0-Li6frac  # lithium-7 fraction
        if Li6frac!=0.0: #Save time if no rubidium-85 required
            Li6atom = AC.Li6
            #Hamiltonian(isotope,transition,gL,Bfield)
            if Dline=='D1D2':
                Li6_ES_D1 = ES.Hamiltonian('Li6','D1',1.0,Bfield)
                Li6_ES_D2 = ES.Hamiltonian('Li6','D2',1.0,Bfield)
                
                # li-6 allowed transitions for light driving sigma minus
                lenergy6D1, lstrength6D1, ltransno6D1 = FreqStren(
                                                        Li6_ES_D1.groundManifold,
                                                        Li6_ES_D1.excitedManifold,
                                                        Li6_ES_D1.ds,Li6_ES_D1.dp,
                                                        'D1','Left',BoltzmannFactor,T+273.16)

                # li-6 allowed transitions for light driving sigma plus
                renergy6D1, rstrength6D1, rtransno6D1 = FreqStren(
                                                        Li6_ES_D1.groundManifold,
                                                        Li6_ES_D1.excitedManifold,
                                                        Li6_ES_D1.ds,Li6_ES_D1.dp,
                                                        'D1','Right',BoltzmannFactor,T+273.16)

                # li-6 allowed transitions for light driving pi
                zenergy6D1, zstrength6D1, ztransno6D1 = FreqStren(
                                                        Li6_ES_D1.groundManifold,
                                                        Li6_ES_D1.excitedManifold,
                                                        Li6_ES_D1.ds,Li6_ES_D1.dp,
                                                        'D1','Z',BoltzmannFactor,T+273.16)
               
                # li-6 allowed transitions for light driving sigma minus
                lenergy6D2, lstrength6D2, ltransno6D2 = FreqStren(
                                                        Li6_ES_D2.groundManifold,
                                                        Li6_ES_D2.excitedManifold,
                                                        Li6_ES_D2.ds,Li6_ES_D2.dp,
                                                        'D2','Left',BoltzmannFactor,T+273.16)

                # li-6 allowed transitions for light driving sigma plus
                renergy6D2, rstrength6D2, rtransno6D2 = FreqStren(
                                                        Li6_ES_D2.groundManifold,
                                                        Li6_ES_D2.excitedManifold,
                                                        Li6_ES_D2.ds,Li6_ES_D2.dp,
                                                        'D2','Right',BoltzmannFactor,T+273.16)

                # li-6 allowed transitions for light driving pi
                zenergy6D2, zstrength6D2, ztransno6D2 = FreqStren(
                                                        Li6_ES_D2.groundManifold,
                                                        Li6_ES_D2.excitedManifold,
                                                        Li6_ES_D2.ds,Li6_ES_D2.dp,
                                                        'D2','Z',BoltzmannFactor,T+273.16)
        ...
        if Li7frac!=0.0:
            ...
            #Hamiltonian(isotope,transition,gL,Bfield)
            if Dline == 'D1D2':
                Li7_ES_D1 = ES.Hamiltonian('Li7','D1',1.0,Bfield)
                Li7_ES_D2 = ES.Hamiltonian('Li7','D2',1.0,Bfield)
                # li-7  allowed transitions for light driving sigma minus
                lenergy7D1, lstrength7D1, ltransno7D1 = FreqStren(
                                                        Li7_ES_D1.groundManifold,
                                                        Li7_ES_D1.excitedManifold,
                                                        Li7_ES_D1.ds,Li7_ES_D1.dp,
                                                        'D1','Left',BoltzmannFactor,T+273.16)

                # li-7 allowed transitions for light driving sigma plus
                renergy7D1, rstrength7D1, rtransno7D1 = FreqStren(
                                                        Li7_ES_D1.groundManifold,
                                                        Li7_ES_D1.excitedManifold,
                                                        Li7_ES_D1.ds,Li7_ES_D1.dp,
                                                        'D1','Right',BoltzmannFactor,T+273.16)
                ...
                # li-7 allowed transitions for light driving sigma plus
                zenergy7D1, zstrength7D1, ztransno7D1 = FreqStren(
                                                        Li7_ES_D1.groundManifold,
                                                        Li7_ES_D1.excitedManifold,
                                                        Li7_ES_D1.ds,Li7_ES_D1.dp,
                                                        'D1','Z',BoltzmannFactor,T+273.16)

                lenergy7D2, lstrength7D2, ltransno7D2 = FreqStren(
                                                        Li7_ES_D2.groundManifold,
                                                        Li7_ES_D2.excitedManifold,
                                                        Li7_ES_D2.ds,Li7_ES_D2.dp,
                                                        'D2','Left',BoltzmannFactor,T+273.16)

                # li-7 allowed transitions for light driving sigma plus
                renergy7D2, rstrength7D2, rtransno7D2 = FreqStren(
                                                        Li7_ES_D2.groundManifold,
                                                        Li7_ES_D2.excitedManifold,
                                                        Li7_ES_D2.ds,Li7_ES_D2.dp,
                                                        'D2','Right',BoltzmannFactor,T+273.16)
                ...
                # li-7 allowed transitions for light driving sigma plus
                zenergy7D2, zstrength7D2, ztransno7D2 = FreqStren(
                                                        Li7_ES_D2.groundManifold,
                                                        Li7_ES_D2.excitedManifold,
                                                        Li7_ES_D2.ds,Li7_ES_D2.dp,
                                                        'D2','Z',BoltzmannFactor,T+273.16)
        ...
        if Dline=='D1D2':
            if (Li6frac!=0.0) and (Li7frac!=0.0):
                ...
                AllEnergyLevels = concatenate((lenergy6D1,lenergy6D2,
                                            renergy6D1,renergy6D2,
                                            zenergy6D1,zenergy6D2))
            elif (Li6frac==0.0) and (Li7frac!=0.0):
                AllEnergyLevels = concatenate((lenergy7D1,lenergy7D2,
                                            renergy7D1,renergy7D2,
                                            zenergy7D1,zenergy7D2))    
...
#Calculate Voigt
    ...
    if Dline!='D1D2':
        #Lithium D1D2 calculation is separated from general cases
        ...
        gamma0 = 2.0*pi*transitionConst.NatGamma*1.e6
        gammaself_D1 = 2.0*pi*gamma0*NDensity*\
                        (transitionConst.wavelength/(2.0*pi))**(3)
        gammaself_D2 = 2.0*pi*gamma0*NDensity*1.414213562373095*\
                        (transitionConst.wavelength/(2.0*pi))**(3)
        gammaD1 = gamma0 + gammaself_D1
        gammaD2 = gamma0 + gammaself_D2
        gammaD1 = gammaD1 + (2.0*pi*GammaBuf*1.e6) #Add extra lorentzian broadening
        gammaD2 = gammaD2 + (2.0*pi*GammaBuf*1.e6) #Add extra lorentzian broadening
        maxShiftedEnergyLevel = amax(abs(AllEnergyLevels)) #integer value in MHz
        voigtwidth = int(1.1*(maxdev+maxShiftedEnergyLevel))
        wavenumber = transitionConst.wavevectorMagnitude
        dipole = transitionConst.dipoleStrength
    ...
    if Elem=='Rb':
        ...
        if Dline!='D1D2':
            ...
            if Li6frac!=0.0:
                lab6, ldisp6, rab6, rdisp6, zab6, zdisp6 = add_voigt_D1D2(d,DoppTemp_li6,
                                                        Li6atom.mass,
                                                        wavenumber,gammaD1,gammaD2,
                                                        voigtwidth,
                                                        ltransno6D1,lenergy6D1,lstrength6D1,
                                                        rtransno6D1,renergy6D1,rstrength6D1,
                                                        ztransno6D1,zenergy6D1,zstrength6D1,
                                                        ltransno6D2,lenergy6D2,lstrength6D2,
                                                        rtransno6D2,renergy6D2,rstrength6D2,
                                                        ztransno6D2,zenergy6D2,zstrength6D2)
            if Li7frac!=0.0:
                lab7, ldisp7, rab7, rdisp7, zab7, zdisp7 = add_voigt_D1D2(d,DoppTemp_li7,
                                                        Li7atom.mass,
                                                        wavenumber,gammaD1,gammaD2,
                                                        voigtwidth,
                                                        ltransno7D1,lenergy7D1,lstrength7D1,
                                                        rtransno7D1,renergy7D1,rstrength7D1,
                                                        ztransno7D1,zenergy7D1,zstrength7D1,
                                                        ltransno7D2,lenergy7D2,lstrength7D2,
                                                        rtransno7D2,renergy7D2,rstrength7D2,
                                                        ztransno7D2,zenergy7D2,zstrength7D2)
            # Make the parts of the susceptibility
            ChiRealLeft= prefactor*(Li6frac*ldisp6+Li7frac*ldisp7)
            ChiRealRight= prefactor*(Li6frac*rdisp6+Li7frac*rdisp7)
            ChiRealZ = prefactor*(Li6frac*zdisp6 + Li7frac*zdisp7)
